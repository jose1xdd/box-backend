...
})), checkSession, checkAuth(['Admin']), weightCategoryController.createWeightCategory);
...
})), checkSession, checkAuth(['Admin']), weightCategoryController.updateWeightCategory);

//get a weight category list
weightCategoryRouter.get('/List', paramsValidator(yup.object().shape({
	minWeight: yup.number().min(0),
	maxWeight: yup.number().min(0),
})), checkSession, checkAuth([]), weightCategoryController.getWeightCategories);

//get a weight category list
weightCategoryRouter.get('/', queryValidator(yup.object().shape({
	weightCategoryId: yup.string().required(),
})), checkSession, checkAuth([]), weightCategoryController.getWeightCategoryById);

//delete a weight category
weightCategoryRouter.delete('/', queryValidator(yup.object().shape({
	weightCategoryId: yup.string().required(),
})), checkSession, checkAuth([]), weightCategoryController.deleteWeightCategory);
